...
export default function TaskDetailsScreen() {
    ...
    const [respond, { isLoading: isResponding }] = useRespondToScheduleMutation();
    const [startConversation] = useStartConversationMutation();

    const [cancelVisible, setCancelVisible] = useState(false);

    // getScheduleById returns the raw schedule (with the whole accommodation
    // populated); the mission-card fields the mapper wants are derived here.
    const task = useMemo(
        () =>
            toCleanerTask({
                ...(schedule ?? {}),
        ...
        <SafeAreaView style={styles.safe}>
            ...
            >
                    ...
                    <View style={styles.mainDateContainer}>
                        ...
                        <View style={styles.dateTimeRow}>
                            <View style={styles.dateBox}>
                                ...
                                <View style={""}>
                                    <Caption4 color={Colors.TEXT_COLOR}>{t("Date")}</Caption4>
                            ...
                            <View style={styles.dateBox}>
                                ...
                                <View style={""}>
                                    <Caption4 color={Colors.TEXT_COLOR}>{t("Time")}</Caption4>
                        ...
                        <View style={styles.infoNote}>
                            ...
                            <Caption4 color={Colors.TEXT_COLOR}> {t("Please arrive within indicated time slot")}</Caption4>
                        </View>
                ...
                <View style={styles.section}>
                    ...
                    <PropertyRow icon={<Caption3><AppertmentIcon /></Caption3>} label={t("Type")} value={task.type} />
                    <PropertyRow icon={<Caption3><SurfaceIcon /></Caption3>} label={t("Surface")} value={task.surface} />
                    <PropertyRow icon={<Caption3><FloorIcon /></Caption3>} label={t("Floor")} value={task.floor} />
                    <PropertyRow icon={<Caption3><BedRoomIcon /></Caption3>} label={t("Rooms")} value={task.rooms} />
                    <PropertyRow icon={<Caption3><BathRoomIcon /></Caption3>} label={t("Bathrooms")} value={task.bathrooms} />
                    <PropertyRow icon={<Caption3><KeyIconIcon /></Caption3>} label={t("Access")} value={task.access} />
                </View>
                ...
                {needsResponse ? (
                    <>
                        <CustomButton
                            ...
                            color='#FF383C'
                            backgroundColor='#FF383C1A'
                            borderRadius={wp(8)}
                            height={hp(52)}
                            width="100%"
                        />
                        <CustomButton
                            title={t("Accept Mission")}
                    ...
                    <>
                        <CustomButton
                            title={t("REPORT A DISPUTE")}
                            icon={<ReportIcon />}
                            onPress={() =>
                                router.push({
                                    pathname: '/cleaner/task/task_detail',
                                    params: { taskId, dispute: '1' },
                                } as any)
                            }
                            color='#4B4B4B'
            ...
            <CancelTaskModal
                ...
                onConfirm={async () => {
                    ...
                    // Cancelling a mission is a refusal from the cleaner's side.
                    await handleRespond('refuse');
                }}
            />
        </SafeAreaView>
    );
