...
export default function CleanerMenuScreen() {
    ...
    // `isActive` is what hosts search against: switching it off takes this
    // cleaner out of the housekeeper results without deleting anything.
    const [isAvailable, setIsAvailable] = useState(true);

    useEffect(() => {
        if (me) setIsAvailable(me.isActive !== false);
    }, [me]);

    const handleAvailabilityChange = async (next: boolean) => {
        setIsAvailable(next); // optimistic — the switch must feel instant
        try {
            const form = new FormData();
            form.append('isActive', String(next));
            await updateMe(form).unwrap();
        } catch (err) {
            setIsAvailable(!next);
            showToast(getApiErrorMessage(err, t("Could not update availability.")), 'error');
        }
    };
    ...
    const CLEANER_SECTIONS = [
        {
            ...
            items: [
                { label: t("Personal Information"), icon: <UserIcon />, route: '/cleaner/profile/personal_info' },
                { label: t("My Clients"), icon: <ClientsIcon />, route: '/cleaner/profile/my_clients' },
                { label: t("Revenue"), icon: <RevenueIcon />, route: '/cleaner/profile/revenue' },
                { label: t("Payment Method"), icon: <PaymentMethodIcon />, route: '/cleaner/profile/payment' },
                { label: t("Password and Security"), icon: <LockIcon />, route: '/cleaner/profile/security' },
                { label: t("Language"), icon: <LanguageIcon />, route: '/cleaner/profile/language' },
                { label: t("Help and Support"), icon: <HeadPhoneIcon color={"#000000"} />, route: '/cleaner/profile/help_support' },
            ],
        },
        {
            title: t("INFORMATION LEGAL"),
            items: [
                { label: t("About Us"), icon: <InfoIcon />, route: '/cleaner/profile/about_us' },
                { label: t("Terms of Use"), icon: <TermsUsesIcon />, route: '/cleaner/profile/terms' },
                { label: t("Privacy Policy"), icon: <PrivacyPolicyIcon />, route: '/cleaner/profile/privacy' },
                { label: t("Legal Notice"), icon: <LegalNoticeIcon />, route: '/cleaner/profile/legal_notice' },
            ],
        },
    ];
    ...
    return (
        <SafeAreaView style={styles.safe}>
            <Body5 color={Colors.PRIMARY_TEXT} style={styles.pageTitle}>{t("Profile")}</Body5>
            ...
            >
                ...
                >
                    ...
                    <View style={styles.profileText}>
                        <Body5 color={Colors.PRIMARY_TEXT} style={styles.name}>
                            {personName(me, 'My profile')}
                        </Body5>
                        <Caption5 color={Colors.TEXT_COLOR}>{t("Housekeeper")}</Caption5>
                    </View>
