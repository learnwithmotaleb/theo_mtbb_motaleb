...
import {
    ...
    resetOnboarding,
    updateOnboarding,
} from '@/redux/slices/cleanerOnboardingSlice';
...
export default function SetupProfileScreen() {
    ...
    const [photoUri, setPhotoUri] = useState<string | null>(draft.avatarUri);
    const [bio, setBio] = useState(draft.biography);
    ...
    const pickImage = async () => {
        ...
        dispatch(
            updateOnboarding({
                avatarUri: asset.uri,
                avatarName: name,
                avatarType: asset.mimeType ?? `image/${ext === 'jpg' ? 'jpeg' : ext}`,
            }),
        );
    };

    // Last step: the five screens land in a single PATCH /auth/update-me.
    const handleContinue = async () => {
        const payload = { ...draft, biography: bio.trim() };
        try {
            await updateMe(onboardingToForm(payload)).unwrap();
            dispatch(resetOnboarding());
            router.push('/cleaner/onboarding/you_are_ready');
        } catch (err) {
            showToast(getApiErrorMessage(err, t("Could not save your profile.")), 'error');
        }
    };
        ...
        <SafeAreaView style={styles.safe}>
            ...
            >
                ...
                <Body6 color={Colors.TEXT_COLOR} style={styles.subtitle}>
                    {t("Let's set up your profile to make it official.")}
                </Body6>
                ...
                <View style={styles.bioLabelRow}>
                    <Body6 color={"#8E8E93"}>{t("Biography")}</Body6>
                ...
                <View style={styles.bioBox}>
                    <TextInput
                        ...
                        value={bio}
                        onChangeText={setBio}
                        placeholder={t("Tell us a little bit about yourself or your professional background...")}
                ...
                <Caption3 color={"#8E8E93"} style={styles.bioHint}>
                    {t("This will be visible to your team and clients.")}
                </Caption3>

                <InfoCard
                    icon={<ShieldCheckIcon size={20} color={Colors.COLOR_ACTIVE} />}
                    title={t("Secure & Private")}
                    description={t("Your data is encrypted and used only for service operational task within Gestlio")}
