...
import { toUiHousekeeper } from '@/lib/mappers';
import { useGetHousekeeperProfileQuery } from '@/redux/services/assignmentApi';
...
export default function HousekeeperDetailScreen() {
    ...
    const { data: profile, isLoading } = useGetHousekeeperProfileQuery(cleanerId, {
        skip: !cleanerId,
    });

    const data = useMemo(
        () => toUiHousekeeper((profile ?? { _id: cleanerId }) as any),
        [profile, cleanerId],
    );
    ...
    return (
        <SafeAreaView style={styles.safe}>
            <SectionTitle title={t("Housekeeper")} />
