...
import { persistLocale, setActiveLocale, useT } from '@/i18n';
import { useAppDispatch, useAppSelector } from '@/redux/hooks';
import { Locale, setLocale } from '@/redux/slices/localeSlice';
...
// Labels stay in their own language, the way language pickers normally do.
...
export function LanguageScreen() {
    ...
    const selected = useAppSelector((state) => state.locale.locale);

    const choose = (code: Locale) => {
        if (code === selected) return;
        // Order matters. The dispatch is what re-renders every mounted screen
        // (they all hold a `t` from useT, which changes identity with the
        // locale); setActiveLocale first so any non-component caller that runs
        // during that render already reads the new catalog.
        ...
        <SafeAreaView style={styles.safe}>
            ...
            <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
                ...
                <Caption3 align="center" color={Colors.TEXT_COLOR} style={styles.subtitle}>
                    {t("Select the language you want to use in the application.")}
                </Caption3>
